...
class Jugador(Programago,ABC):
    ...
    def celebrar(self):
        print(self.nombre + "GANEE<3 gracias por el apoyo :'")
        return
...
class Buscador(Jugador):
    ...
    def competir(self):
        valor = self.velocidad * p.PONDERADOR_VELOCIDAD_BUSCADOR + self.equilibrio * p.PONDERADOR_EQUILIBRIO_BUSCADOR - self.nerviosismo * p.PONDERADOR_NERVIOSISMO_BUSCADOR
        return valor
...
class Golpeador(Jugador):
    ...
    def competir(self):
        valor = self.velocidad * p.PONDERADOR_VELOCIDAD_GOLPEADOR + self.equilibrio * p.PONDERADOR_EQUILIBRIO_GOLPEADOR - self.nerviosismo * p.PONDERADOR_NERVIOSISMO_GOLPEADOR
        return valor
...
class Cazador(Jugador):
    ...
    def competir(self):
        valor = self.velocidad * p.PONDERADOR_VELOCIDAD_CAZADOR + self.equilibrio * p.PONDERADOR_EQUILIBRIO_CAZADOR - self.nerviosismo * p.PONDERADOR_NERVIOSISMO_CAZADOR
        return valor
