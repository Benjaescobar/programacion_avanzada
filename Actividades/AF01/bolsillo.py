...
class BolsilloCriaturas(list):
    ...
    def append(self, criatura):
        # Completar
        if len(self) < 6:
            self.insert(len(self),criatura)
            return
        else:
            print("Error, ya tienes 6 criaturas")
            return
        

    def cantidad_criaturas_estrella(self):
        # Completar
        criaturas_estrellas = 0
        for i in self:
            if i.hp_base + i.atk + i.sp_atk + i.defense > 400:
                criaturas_estrellas += 1
            else:
                continue
        return criaturas_estrellas
